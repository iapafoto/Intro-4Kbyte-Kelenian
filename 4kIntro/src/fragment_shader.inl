...
"in vec2 COORD;"
"vec2 iResolution=vec2(1280,1024);"
"vec4 mins,maxs;"
"vec2 kColor;"
...
"float c,r=m;"
"for(int l=0;l<132;l++){"
"c=p(v+x*r);if(c<f*r||r>y)break;r+=c;"
"}"
"return r;"
"}"
...
"}"
"void main() {"
"vec2 B = 0.5 * COORD + 0.5;"
"float v=.1*fpar[0].x,c=smoothstep(0.,1.,fract(v));"
"float F=smoothstep(.7,1.,fract(v*127./6.));"
"vec2 d=iResolution.xy;"
"int r=int(v)%16,m=r+1;"
...
"kColor=mix(vec2(.25,1.),vec2(.01325,1.23), F*F*(.13+smoothstep(10.,12.,v)));"
"vec3 g=rd(x,l,y,n,B,d);"
"vec4 p=f(x,g);"
"p.xyz*=pow(16.*B.x*B.y*(1.-B.x)*(1.-B.y),.3);"
"dt(mod(v,float(16)),B,p);"
"color=p;"
"}";
